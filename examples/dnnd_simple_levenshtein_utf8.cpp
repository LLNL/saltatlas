...
// Use string with 32 bits per character as points
// using point_type = std::u32string;
...
int main(int argc, char **argv) {
  ...
  auto levenshtein_utf8 = [](const point_type &a, const point_type &b) {
    std::wstring_convert<std::codecvt_utf8<char32_t>, char32_t> converter;

    std::u32string a32 = converter.from_bytes(a);
    std::u32string b32 = converter.from_bytes(b);
  ...
  if (!opt.query_file_path.empty() && opt.query_k > 0) {
    ...
    const auto [query_results, ngbr_points] = g.query_with_features(

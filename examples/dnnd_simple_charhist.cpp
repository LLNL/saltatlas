...
Usage example:
cd saltatlas/build
mpirun -n 2 ./examples/dnnd_simple_charhist -p str -k 4 -G ./knng-dump \
...
// L1 distance between the histograms of the two strings.
dist_t char_histgram_distance(const point_type &p1, const point_type &p2) {
  // Compute the histogram of characters in p1 and p2
  int hist1[256] = {0};
  int hist2[256] = {0};
  for (const auto &c : p1) {
    hist1[c]++;
  ...
  // Calculate the L1 distance between the histograms
  dist_t d = 0;
  for (std::size_t i = 0; i < 256; ++i) {
    const auto x = std::abs(hist1[i] - hist2[i]);
    d += x;
  }
  return static_cast<double>(std::sqrt(d));
}

...
void fill_seed_vector(const std::vector<size_t>      &seed_ids,
  ...
  auto      store_seed_features_lambda =
      [](auto mailbox, size_t seed_index, std::vector<float> vals,
      ...
      while (std::getline(ifs, line) && curr_seed_ptr < seed_ids.size()) {
        ...
        if (index == seed_ids[curr_seed_ptr]) {
          ...
          for (size_t i = 0; i < 8; ++i) {
            ssline >> val;
            vals.push_back(val);
            // seed_features[curr_seed_ptr].push_back(val);
          }
  ...
  // This is made more difficult by storing seeds as a vector of vectors
...
void dpockets_query_trial(
  ...
  auto count_lambda = [](const std::vector<float>           &query_pt,

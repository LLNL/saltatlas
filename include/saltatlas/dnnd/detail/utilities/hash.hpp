...
namespace saltatlas::dndetail::murmurhash {
...
inline __attribute__((always_inline)) uint64_t getblock64(const uint64_t *p,
                                                          int             i) {
  return p[i];
}
...
struct hash {
  ...
  inline std::size_t operator()(const T &key) const noexcept {
    uint64_t hashed_value[2];
    MurmurHash3_x64_128(&key, sizeof(T), seed, hashed_value);
    return hashed_value[0];
  }
};

}  // namespace saltatlas::murmurhash